...
<main class="ml-64 relative">
    <div class="container mx-auto">
        ...
        %>
            <h2 class="text-2xl font-bold mb-6">List product price</h2>
            ...
            <table class="table-auto w-full">
                <thead>
                    <tr class="text-left font-semibold text-gray-600 uppercase">
                        <th class="py-2 px-4">Medicine name</th>
                ...
                List<HistoPrice> hps = (List<HistoPrice>) request.getAttribute("histoPrice");

                    for(HistoPrice hp: hps)
                    {


                    %>
                    <tr class="border-b border-gray-200 cursor-pointer">
                        <td class="py-2 px-4"><%= hp.getDosage().getDisplayName()  %></td>
                        <td class="py-2 px-4"><%= hp.getDatePrice()  %></td>
                        <td class="py-2 px-4"><%= hp.getPrice() %></td>
...
<div id="filterSidebar" class="w-80 h-screen bg-white shadow-lg z-100 p-6" style="position: fixed; top: 0; right: 0; transform: translateX(100%); z-index: 200; display: flex; flex-direction: column; justify-content: space-between;">
    ...
    <form action="pricing" method="get" id="filterForm" style="flex-grow: 1; display: flex; flex-direction: column; justify-content: space-between;">
        <div>
            ...
            <div class="mb-4">
                <label for="dateMin" class="block text-gray-700 font-bold mb-2">Date min :</label>
                <input type="date" id="dateMin" name="dateMin" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="">
            </div>
            <div class="mb-4">
                <label for="dateMax" class="block text-gray-700 font-bold mb-2">Date max :</label>
                <input type="date" id="dateMax" name="dateMax" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="">
            </div>
